...
if (isset($_POST['login'])) {
  ...
  if (!$select_all_users) {
    ...
    exit();
     ...
     if($select_all_users){
     $row = mysqli_num_rows($select_all_users); 
     echo $row;
     }                        
...
if ($row == 1) {
                ...
                $logged_in_query="UPDATE login SET LoggedTime ='{$current_time}', signed_in='{$_SESSION['signed_in']}'  WHERE  username='{$username}' && password='{$password}'";
  ...
  if (!$logged_in_query) {
        die('query FAILED . mysqli_error($connection)');
                 ...
                 $_SESSION['signed_in']=true;
                  $current_time = date('y-m-d h:m:s');
                 $_SESSION['loggedTime']=date('y-m-d h:m:s');
                $logged_in_query="UPDATE newuser SET LoggedTime ='{$current_time}', signed_in='{$_SESSION['signed_in']}'  WHERE  username='{$username}' && password='{$password}'";
