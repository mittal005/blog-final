...
while($row=mysqli_fetch_array($select_allposts)) {
  ...
  if ($Status=="Publish") {
   $query="UPDATE pages SET Title='$title',categoryName='$category',seo_title='$fname',publishTime='$fdate',createdDate='$fdate' WHERE id={$id}";
  ...
  if ($Status=="Draft") {
   $query="UPDATE pages SET Title='$title',categoryName='$category',seo_title='$fname', draftTime='$fdate',createdDate='$fdate' WHERE id={$id}";
  $update_query=mysqli_query($connection,$query);
  if (!$update_query) {
    printf("Error: %s\n", mysqli_error($connection));
    exit();
...
<!-- <a href="#edit<?php //echo $row['userid']; ?>" data-toggle="modal" class="btn btn-warning"><span class="glyphicon glyphicon-edit"></span> Edit</a>  -->
  ...
  <!-- <button class="quickEditPost" >edit</button> -->
    <a class="dropdown-item deletePost" data-title="Delete" data-id="<?php echo $id;  ?>">Trash&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i class="fa fa-trash padd"></i></a>
             </div>
</label>
</td>
    </tr>
