...
while($row=mysqli_fetch_array($select_allposts)) {
  ...
  if ($Status=="Publish") {
   $query="UPDATE posts SET Title='$title',seo_title='$fname',categoryName='$category',publishTime='$fdate' WHERE id={$id}";
  ...
  if ($Status=="Draft") {
   $query="UPDATE posts SET Title='$title',seo_title='$fname',categoryName='$category', draftTime='$fdate' WHERE id={$id}";
  $update_query=mysqli_query($connection,$query);
  if (!$update_query) {
    printf("Error: %s\n", mysqli_error($connection));
    exit();
