...
while($row=mysqli_fetch_array($select_allposts)) {
  ...
  if ($Status=="Publish") {
   $query="UPDATE pages SET Title='$title',seo_title='$fname',categoryName='$category',publishTime='$fdate',createdDate='$fdate' WHERE id={$id}";
  ...
  if ($Status=="Draft") {
   $query="UPDATE pages SET Title='$title',seo_title='$fname',categoryName='$category', draftTime='$fdate',createdDate='$fdate' WHERE id={$id}";
  $update_query=mysqli_query($connection,$query);
  if (!$update_query) {
    printf("Error: %s\n", mysqli_error($connection));
    exit();
