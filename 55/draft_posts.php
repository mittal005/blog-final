<?php
 ...
 if (isset($_POST['update_post'])) {
   ...
   $title=$_POST['ftitle'];
    ...
    $draftTime=$_POST['draftTime'];
     $status=$_POST['status'];
   //$password=$_POST['fdate'];
  
   $query="UPDATE posts SET Title='$title',username='$username',draftTime='$draftTime',status='$status' WHERE id={$id}";
  ...
  $publishTime=$row['publishTime'];
    ...
    <td> <a class="nav-link " href="#" id="actionmenu" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        ...
        <div class="dropdown-menu dropdown-menu-right" id="actionmenulist" aria-labelledby="actionmenu">
          <a class="dropdown-item quickEditPost" href="edit_draft_post.php?edit=<?php echo $id;   ?>">Edit<i class="fa fa-pencil pad_left_40"></i
            ></a>
...
<div class="modal fade" id="quickeditmodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
  <div class="modal-dialog" role="document">
   ...
   <label>slug</label>
   <input type="text" class="form-control" name="username" id="content" required>
    <label>Date</label>
           <input data-date-format="dd/mm/yyyy"name="draftTime" id="datepicker" name="fdate" class="form-control">
       <label>Status</label>
