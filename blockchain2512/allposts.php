...
while($row=mysqli_fetch_array($select_allposts)) {
  ...
  if ($Status=="Publish") {
   $query="UPDATE posts SET Title='$title',username='$fname',publishTime='$fdate',onlyDate='$fdate1' WHERE id={$id}";
  ...
  if ($Status=="Draft") {
   $query="UPDATE posts SET Title='$title',username='$fname', draftTime='$fdate',onlyDate=' $fdate1' WHERE id={$id}";
  $update_query=mysqli_query($connection,$query);
  if (!$update_query) {
    printf("Error: %s\n", mysqli_error($connection));
    exit();
