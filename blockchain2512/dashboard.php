...
$select_query="SELECT * FROM posts ORDER BY id DESC LIMIT 5";
    ...
    exit();
                   ...
                   ?>  
       <a href="editpost.php?edit=<?php echo $id;  ?>"><label><?php echo $Title; ?></label></a> <span>  <?php echo $publishTime;  ?> </span>
...
<form action="" method="post">
 ...
 <button class="btn btn-primary button-top"name="save_draft">Save Draft</button>
   <?php
 $select_query="SELECT * FROM quickdraft ORDER BY id DESC LIMIT 3";
                $select_all_categories_query=mysqli_query($connection, $select_query);
                if (!$select_all_categories_query) {
    printf("Error: %s\n", mysqli_error($connection));
    exit();
...
<div id="recentdraft">
 <a class="dropdown-item edit_data" name="edit" id="<?php echo $id; ?>" ><?php echo $show_title;    ?></a>

 <label><?php echo  $show_time;   ?></label>
 <!-- <input type="button" name="edit" value="Edit" id="<?php echo $id; ?>" class="btn btn-info btn-xs edit_data" /> -->
</div>
<?php
 }  
?>
...
<!-- <div class="fetched-data"></div>  -->
  <!--  <input type="text" name="fetched-data" id="bookId" value="">  -->
