 ...
 if (isset($_POST['restore_post'])) {
   ...
   if (!$select_query) {
    ...
    $fid_insert="INSERT INTO category(selectCategory,categoryName,parentcategory)VALUES('$selectCategory','$categoryName','$categoryType')";
...
if ($Status=='Publish') {
  $query="INSERT INTO posts(fid,user_id,username,Title,Content,Status,publishTime,Visibility,Format,seo_title,url,seo_schema,image,onlyDate)VALUES('$fid','$userid','$username','$Title','$Content','$Status','$current_time','$Visibility','$Format','$seo_title','$url','$seo_schema','$image','$current_time')";
...
if ($Status=='Draft') {
  $query="INSERT INTO posts(fid,user_id,username,Title,Content,Status,draftTime,Visibility,Format,seo_title,url,seo_schema,image,onlyDate)VALUES('$fid','$userid','$username','$Title','$Content','$Status','$current_time','$Visibility','$Format','$seo_title','$url','$seo_schema','$image','$current_time')";
    $insert_query=mysqli_query($connection,$query);
    if (!$insert_query) {
    printf("Error: %s\n", mysqli_error($connection));
    exit();
...
<!-- <a href="#edit<?php //echo $row['userid']; ?>" data-toggle="modal" class="btn btn-warning"><span class="glyphicon glyphicon-edit"></span> Edit</a>  -->
  ...
  <!-- <button class="quickEditPost" >edit</button> -->
    <a class="dropdown-item deletePost" data-title="Delete" data-id="<?php echo $id;  ?>">Delete permanently<i class="fa fa-trash padd"></i
            ></a>
             </div>
