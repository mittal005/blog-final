...
$select_by_author_query="SELECT * FROM pages  WHERE  createdDate='{$publihedOn}'";
  ...
  $select_by_author_query1="SELECT selectCategory FROM category  WHERE  id='{$fid}'";
   $show_by_author_query2=mysqli_query($connection,$select_by_author_query1);  
   if (!$show_by_author_query2) {
    printf("Error: %s\n", mysqli_error($connection));
    exit();
  }  
  
while($row=mysqli_fetch_array($show_by_author_query2)) {
...
$selectCategory=$row['selectCategory'];
  ...
  <tr>
    ...
    <td><label><?php echo $createdDate;    ?></label></td>
