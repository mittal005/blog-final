...
<?php
if(isset($_PAGES['saveDraft']))
{
  
    $username=$_SESSION['username'];

    $addTitle=$_PAGES['addTitle'];
    $pageContent=$_PAGES['pageContent'];
     $statusdropdown=$_PAGES['statusdropdown'];
        $visible=$_PAGES['visible1'];
        $x=$_PAGES['x'];
    date_default_timezone_set("Asia/Kolkata");
     $current_time = date("F j, Y, g:i a");
     $category_name=$_PAGES['category_name'];
     $category_type=$_PAGES['category_type'];
      $seo_title=$_PAGES['seo_title'];
    $description=$_PAGES['description'];
     $url=$_PAGES['url'];
     $schema=$_PAGES['schema'];


  $connection=mysqli_connect('localhost','root','','admin4');
  if ($statusdropdown == 'Draft') {
 $query="INSERT INTO pages(username,Title,Content,Status,Visibility,Format,draftTime,categoryName,categoryType,seo_title,description, url,seo_schema)VALUES('$username','$addTitle','$pageContent','$statusdropdown','$visible','$x','$current_time','$category_name','$category_type','$seo_title','$description','$url','$schema')";
...
if ($statusdropdown == 'Publish') {
 $query="INSERT INTO pages(username,Title,Content,Status,Visibility,Format,publishTime,categoryName,categoryType,seo_title,description, url,seo_schema)VALUES('$username','$addTitle','$pageContent','$statusdropdown','$visible','$x','$current_time','$category_name','$category_type','$seo_title','$description','$url','$schema')";
  $create_page_query=mysqli_query($connection,$query);
  if (!$create_page_query) {
...
if(isset($_PAGES['add_new_category']))
{
...
<div class="row dash_row">
  ...
  <hr>
    ...
    <div  contentEditable="true" data-text="Enter">
      <textarea id='edit' name="pageContent"></textarea>
       </div>
  </div>
...
<i class="fa fa-question-circle pad"></i>Title 
<form>
<input type="text" class="pad form-control" name="ta1"onKeyDown="textCounter1(this.form.ta1,this.form.countDisplay1);" onKeyUp="textCounter1(this.form.ta1,this.form.countDisplay1);"><br>
<input readonly type="text" name="countDisplay1" size="3" maxlength="3" value="0"><span> Characters Remaining</span><br>
</form><br>
<i class="fa fa-question-circle pad"></i>Description
<form>
<textarea class="form-control pad" name="ta"onKeyDown="textCounter(this.form.ta,this.form.countDisplay);" onKeyUp="textCounter(this.form.ta,this.form.countDisplay);"></textarea>
<br>
<input readonly type="text" name="countDisplay" size="3" maxlength="3" value="0"> <span>Characters Remaining</span>
</form>
<br>
...
<div class="modal fade bd-example-modal-lg" id="fimage" tabindex="-1" role="dialog" aria-labelledby="fimageTitle" aria-hidden="true">
 ...
 <div class="row">
  <div class="col-md-4 col-offset-md-8">
    <a href="#" class="pad" onclick="featuremeida()">Upload image</a> <a href="#" class="pad" onclick="media()">Media library</a>
  </div>
</div>
